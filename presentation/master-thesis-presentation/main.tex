...
\begin{itemize}
    ...
    \item meje med različnimi vlogami članov se postopno zabrisujejo
...
% \column{.6\textwidth} % Left column and width
\begin{itemize}
    \item študija primerov iz resničnih projektov
    \item prilagodljiva komponenta za vzpostavitev cevovoda CI/CD
    \item odprtokodni vtičnik za integrirano okolje IntelliJ IDEA, ki preverja pravilnost konfiguracijskih datotek GitLab CI/CD
\end{itemize}
...
\note[item]{Cilj magistrskega dela je predstaviti izzive pri razvoju poslovno kritičnih aplikacij in dobre prakse razvoja programske opreme, ki nam pomagajo pri reševanju teh izzivov.}
\note[item]{Na podlagi študije primerov iz resničnih projektov smo razvili prilagodljivo komponento za vzpostavitev cevovoda CI/CD, ki poenoti in poenostavi vzpostavitev cevovoda znotraj podjetja kot tudi pri naročniku.}
\note[item]{Dodatno smo razvili tudi odprtokodni vtičnik za integrirano okolje Intellij IDEA, ki znotraj okolja preverja konfiguracijske datoteke in prikazuje združeno vsebino vseh dodanih predlog.}
\end{frame}
...
\note[item]{orodje za gradnjo, ki je odgovorno za avtomatsko gradnjo in objavljanje posodobljene kode}
\note[item]{orodje za preizkušanje, ki izvaja avtomatsko testiranje na posodobljeni kodi in s tem zagotavlja, da ta deluje pravilno in dosega zahtevane standarde kakovosti}
...
\column{.5\textwidth}
\begin{figure}[]
\includegraphics[scale=0.1, right]{figs/osnoven-primer-cevovoda.png}
\label{slika1}
\end{figure}
\end{columns}
...
\note[item]{Orodje temelji na konceptu cevovodov, ki so sestavljeni iz zaporedja nalog. Vsak cevovod ima lahko več faz, v katerih je lahko več nalog, vsaka naloga pa ima lahko več akcij.}
\note[item]{Cevovod definiramo v datoteki \texttt{.gitlab-ci.yml}, ki se mora nahajati v korenskem imeniku projekta. Orodje to datoteko avtomatsko prepozna in ob vsakem odrivu kode v repozitorij kreira cevovod.}
\note[item]{Za izvajanje nalog skrbijo izvajalci nalog GitLab. Izvajajo se lahko na lastni infrastrukturi ali pa na oblačnih virtualnih strojih in omogočajo vzporedno izvajanje nalog, kar lahko znatno pohitri izvajalni čas cevovoda.}
\end{frame}
...
\begin{itemize}
    \item izdelava komponente Medius CD
    \item vtičnik Gitlab Template Lint
    \item študija primera z uporabo komponente
\end{itemize}

\note[item]{Sedaj se bomo osredotočili na delo, ki smo ga opisali v magistrski nalogi in predstavili rezultate zastavljenih ciljev.}
...
\note[item]{Osnoven cevovod v našem podjetju je sestavljen iz naslednjih korakov:}
...
\note[item]{Naročnik lahko nato ročno zažene posebno nalogo, ki prenese spremembe v njegov repozitorij.}
...
\begin{itemize}
    \item poenotenje cevovodov, zmanjšanje količine podvojene konfiguracije in kode ter lažje vzdrževanje
...
\vspace{20px}
\begin{figure}[b]
\includegraphics[scale=0.07]{figs/medius-cd.png}
\label{slika1}
\end{figure}
...
\column{.5\textwidth}
\begin{figure}[b]
\includegraphics[scale=0.17]{figs/config_file_2.png}
\label{slika1}
\end{figure}
...
\begin{itemize}
    ...
    \item logiko kompleksnejših nalog napišemo v samostojnih skriptih v jeziku Bash
...
\note[item]{imenik \texttt{deploy/k8s}, ki vsebuje datoteke s konfiguracijo za namestitev na gručo na platformi Kubernetes}
\note[item]{vnaprej pripravljeno datoteko \texttt{.gitlab-ci.yml}, ki je za vse projekte enakam ki pogojno uporabi eno izmed predlog v imeniku \texttt{.gitlab}}
\note[item]{imenik \texttt{.gitlab}, z datotekama \texttt{.build-runners.yml}, ki definira cevovod za gradnjo izvajalnih slik in \texttt{.main.yml}, ki definira cevovod aplikacije}
...
\begin{figure}[b]
\includegraphics[scale=0.23]{figs/java_config_file_2.png}
\label{slika1}
\end{figure}
...
\begin{itemize}
    ...
    \item omogočili podporo za različne programske jezike
