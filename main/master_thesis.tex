...
\begin{itemize}
    \item Razvijalec prebere opis naloge in doda vprašanja glede razvoja.
    \item Ko se vprašanja razrešijo, si kreira novo repozitorijsko vejo na kateri bo nalogo razvil.
    ...
    \item Ob vsakem odrivu aplikacijske kode, se izvede cevovod, ki preveri njegovo kodo.
    \item Ko razvijalec konča z razvojem naloge, kreira zahtevek za združitev v spletni platformi Gitlab in nanj dodeli pregledovalca.
...
\begin{itemize}
    ...
    \item Stranka zažene posebno nalogo, ki je del strankinega cevovoda in prenese spremembe v njihov repozitorij.
...
\end{itemize}
Na ta način smo stranki omogočili, da lahko sama enostavno namesti aplikacijo v svoja okolja in je na ta način bolj neodvisna od podjetja, ki je aplikacijo razvilo.

V kolikor stranka ne uporablja spletne platforme Gitlab za upravljanje in nadzor različic aplikacij, pa se dogovorimo za drugačen način prevzema sprememb in namestitve aplikacij v strankina okolja.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{dir} - korenski imenik aplikacije.
        \item \texttt{variables} - spremenljivke, ki so na voljo v nalogah, ki se tičejo te aplikacij.
    ...
    \begin{itemize}
        \item \texttt{variables} - spremenljivke, ki so na voljo pri namestitvi okolja za integracijske teste
...
\end{verbatim}
\end{itemize}
V nadaljevanju opišemo posamezne naloge v standardnem cevovodu.


\subsection{Inicializacija komponente Medius CD}
Prva naloga vseh cevovodov je inicializacija komponente Medius CD. Naloga poskusi najprej preberati datoteko \texttt{config\_file.yaml} iz konfiguracijskega strežnika ali Gitlab variable. V kolikor datoteka obstaja, pripravi dve novi datoteki, ki sta dodani kot artefakta:
\begin{itemize}
    ...
    \item \texttt{config.env} - datoteka, ki ima v vsaki vrstici po eno spremenljivko in njeno vrednost, ločeno z enačajem. To so tiste spremenljivke, ki morajo biti na voljo v vseh nalogah. To datoteko nastavimo kot poseben artefakt pod atribut \texttt{artifacts.reports.dotenv}, ki na začetku vsake naloge, ki je odvisna od te naloge, nastavi vse spremenljivke v tej datoteki kot okoljske spremenljivke.
\end{itemize}
...
Po uspešni gradnji slik Docker lahko ročno zaženemo nalogo za integracijske teste. Zaradi daljšega trajanja jih ne želimo avtomatsko zagnati v vseh cevovodih. Za testiranje integracije potrebujemo postavljeno testno bazo, če aplikacija za delovanje potrebuje bazo, aplikacijo in vse aplikacije, na katere se aplikacija integrira. Zato  je naloga, ki zažene integracijske teste, tipa \texttt{trigger} in kreira otroški cevovod, ki je definiran s konfiguracijsko datoteko Gitlab trenutnega projekta, vendar nastavljeno spremenljivko Gitlab \texttt{IT\_FORCE} na \texttt{true}. To povzroči, da otroški cevovod vsebuje naslednje naloge:
\begin{itemize}
    \item \texttt{initialize-database} - na podlagi datotek , ki so dodane v repozitorij, pripravi bazo
    ...
    \item \texttt{prepare-deploy-it} - naloga tipa \texttt{trigger}, ki na podlagi konfiguracije pod ključno besedo \texttt{it} v konfiguracijski datoteki \texttt{config\_file}, kreira otroški cevovod z nalogami, ki namestijo vse odvisne aplikacije. Naloge za namestitve drugih aplikacij, so zopet tipa \texttt{trigger}, saj zaženejo cevovod druge aplikacije s tako nastavljenimi spremenljivkami Gitlab, da se namestijo v enako okolje kot testirana aplikacija.
    \item \texttt{test} - po uspešni namestitvi aplikacij s pomočjo orodja za testiranje zažene integracijske teste
    \item \texttt{clean-up-database} - po končanem izvajanju testov izbriše bazo, ki je bila kreirana za namen integracijskih testov.
    \item \texttt{clean-up-namespace} - po končanem izvajanju testov izbriše okolje na platformi Kubernetes, ki je bilo uporabljeno za namestitev aplikacij.
\end{itemize}
...
\begin{itemize}
    ...
    \item Vse spremenljivke v datoteki zamenja z vrednostmi, ki jih imajo okoljske spremenljivke z enakim imenom, vendar brez podčrtajev npr. \texttt{SERVICE\_NAME} in ustvari novo konfiguracijsko datoteko za namestitev na platformi Kubernetes imenovano \texttt{deployment.yml}.
    \item S programom \texttt{kubectl} in ustvarjeno datoteko kreira novo namestitev na platformi Kubernetes.
    \item S programom \texttt{kubectl} dokler aplikacija ni uspešno nameščena ali pa preteče 10 minut preverja stanje namestitve.
\end{itemize}
...
\chapter{Rezultati}
\label{ch:rezultati}
V poglavju bomo predstavili uporabo razvite komponente na resničnih projektih, ki se še razvijajo ali pa so že v fazi produkcije. Najprej bomo opisali kako smo izboljšali pripravo zagonskih slik. Nato bomo predstavili še eno prednost uporabe konfiguracijske datoteke \texttt{config\_file}. Na koncu bomo predstavili kako lahko z uporabo razvite komponente enostavno postavimo cevovod projekta za razvoj aplikacije in projekta za razvoj programske knjižnice.
...
\begin{itemize}
    \item Imenik \texttt{runner-images} v katerega dodamo datoteko Docker, ki vsebuje ukaze za namestitev orodji, ki jih potrebujemo za gradnjo aplikacije in aplikacijske slike Docker. Za osnovno lahko uporabimo pripravljeno sliko \texttt{rocky-with-tools}, ki že vsebuje vsa osnovna orodja potrebna za izvajanje nalog.
    ...
    \item Imenik \texttt{deploy/k8s} v katerega dodamo datoteko \texttt{deployment.yml.in}, ki vsebuje konfiguracijo za namestitev na gručo na platformi Kubernetes.
...
Za postavitev cevovoda za razvoj programske knjižnice potrebujemo enake imenike in datoteke, z izjemo imenika \texttt{deploy}, ki ga ne potrebujemo. Tudi vsebina datotek ostane v večini enaka, edina razlika je v datoteki \texttt{.main.yml}, v kateri moramo uporabiti predlogo cevovoda za programsko knjižnico.

\section{Projekt v programskem jeziku Java}
\label{sec:java-projekt}
V tem poglavju bomo predstavili postavitev cevovoda projekta, ki je napisan v programskem jeziku Java in za gradnjo uporablja orodje Maven. Aplikacija uporablja aplikacijski strežnik Wildfly in preko programskega vmesnika izpostavlja končne točke za izvajanje poslovnih operacij. Za shranjevanje podatkov uporablja podatkovno bazo Oracle.

Stranka ima postavljeno svojo instanco platforme Gitlab na kateri se cevovod izvaja. Zaradi varnosti je stranka zahtevala, da se celoten cevovod izvaja brez povezave na internet. To pomeni, da morajo biti vse odvisnosti projekta dostopne preko internih repozitorijev in registrov.

\subsection{Cevovod v podjetju}
\label{subsec:java-projekt-cevovod-v-projektu}
Ker želimo, da je cevovod v podjetju čim bolj podoben cevovodu pri stranki, smo pripravili izvajalec nalog Gitlab, ki nima dostopa do interneta. Dodali smo mu posebno značko in jo shranili v spremenljivko Gitlab imenovani \texttt{RUNNER\_TAGS}. Na ta način razviti cevovod ve katerega izvajalca nalog mora uporabiti. 
...
services:
  - environments:
      ...
      - test3
\end{verbatim}
Najprej nastavimo nekaj globalnih spremenljivk, ki se uporabijo v vseh nalogah cevovoda in spremenijo privzete nastavitve:
\begin{itemize}
    \item \texttt{KUBECONFIG\_CONSUL\_PATH} - pot do konfiguracijske datoteke \texttt{kubeconfig}, ki vsebuje podatke za povezavo in avtentikacijo s platformo Kubernetes na katero želimo aplikacijo namestiti.
    \item \texttt{K8S\_DEPLOY\_DIR} - pot do imenika v repozitoriju, kjer se nahajajo konfiguracijske datoteke za namestitev aplikacije z uporabo orodja \texttt{kubectl}. Na ta način lahko vzdržujemo različne konfiguracijske datoteke, ki se uporabljajo za namestitve na različne verzije gruč Kubernetes.
...
Pri stranki se uporablja enak cevovod vendar brez uporabe konfiguracijskega strežnika Consul. To pomeni, da morajo vse datoteke, ki se sicer prenesejo iz konfiguracijskega strežnika, shraniti v spremenljivke Gitlab tipa datoteka. Nastaviti morajo naslednje spremenljivke:
\begin{itemize}
    \item \texttt{DNF\_REPOS\_FILE} - Nastavitve za povezavo in avtentikacijo z repozitoriji DNF.
    \item \texttt{MAVEN\_SETTINGS\_FILE} - Nastavitve za povezavo in avtentikacijo z orodjem Maven na centralni repozitorij.
    \item \texttt{CONFIG\_FILE} - Vsebina datoteke je zelo podobna tisti, ki se uporablja v podjetju. Razlikuje se zgolj v okoljih, ki jih stranka uporablja za namestitve in imenih spremenljivk Gitlab, ki vsebujejo datoteke za konfiguracijo povezave in avtentikacije na platformo Kubernetes.
...
Ker med izvajanjem cevovoda nimamo dostopa do interneta, moramo najprej ročno prenesti naslednji sliki Docker: 
\begin{itemize}
    \item \texttt{kaniko-tools} - vsebuje orodje \texttt{kaniko} in osnovna orodja potrebna za gradnjo slik Docker.
    \item \texttt{rocky-with-tools} - se uporablja za osnovno pri gradnji zagonskih in aplikacijskih slik.
\end{itemize}
Nato je potrebno sliki naložiti v register slik, ki je dostopen med izvajanjem cevovoda in nastaviti naslednji spremenljivki Gitlab:
\begin{itemize}
    \item \texttt{DOCKER\_BUILDER\_IMAGE} - naslov do slike \texttt{kaniko-tools}.
...
\end{itemize}
V imenik \texttt{runner-images} smo dodali dve datoteki Docker, ki definirata dve zagonski sliki:
\begin{itemize}
    \item \texttt{jdk} - vsebuje razvojno okolje Java in orodje Maven. Uporablja se za zagon vseh nalog v cevovodu.
    \item \texttt{wildfly} - osnova za gradnjo aplikacijske slike Docker, ki prav tako vsebuje razvojno okolje Java in orodje Maven ter dodatno še aplikacijski strežnik Wildfly.
\end{itemize}
Obe datoteki pod ključno besedo \texttt{FROM} uporabita spremenljivko \texttt{DOCKER\_FROM\_REGISTRY}, ki smo jo shranili kot spremenljivko Gitlab in se pri gradnji slik poda kot argument gradnje. Na ta način se za osnovo slik uporabijo slike iz registra, ki je dostopen med izvajanjem cevovoda.
...
Za definicijo cevovoda smo v korenski imenik dodali datoteko \texttt{.gitlab-ci.yml}, kot omenjeno v poglavju \ref{sec:projekt-aplikacije}, in uporabili vnaprej pripravljeni predlogi, ki smo ju dodali v datoteki v imeniku \texttt{.gitlab}. V datoteki \texttt{.gitlab/main.yml} smo uporabili predlogo za cevovod projekta, ki uporablja programski jezik Java in orodje Maven:
\begin{verbatim}
include:
  - project: 'medius/ci-tools/medius-cd'
    ref: v4.5.1
...
Za zagon cevovoda aplikacije potrebujemo zagonske slike. Zgradimo jih lahko tako, da poženemo cevovod za gradnjo zagonskih slik. To naredimo tako, da pred zagonom cevovoda nastavimo vrednost spremenljivke \texttt{BUILD\_RUNNERS} na \texttt{true}. Zažene se cevovod, ki ima dva koraka: inicializacija, ki vsebuje nalogo za pripravo komponente Medius CD, in gradnja zagonskih slik, ki vsebuje nalogi za gradnjo zagonskih slik \texttt{jdk} ter \texttt{wildfly}. Cevovod lahko vidimo na sliki \ref{fig:java-projekt-gradnja-zagonskih-slik}.
...
Nato lahko zaženemo cevovod aplikacije, ki ga lahko vidimo na sliki \ref{TODO} in vsebuje naslednje naloge:
\begin{itemize}
    \item priprava komponente Medius CD
    \item gradnja aplikacije
    \item gradnja slik Docker, ki dinamično kreira naloge z v otroškem cevovodu,
    \item preverjanje kakovosti, ki jo lahko ročno zaženemo,
    \item izdaja slik Docker
    \item namestitev aplikacije v različna okolja, ki dinamično kreira naloge v otroškem cevovodu,
    \item naloge za izdajo nove verzije
\end{itemize}

\section{Projekt v programskem jeziku TypeScript}
\label{sec:typescript-projekt}
V tem poglavju bomo predstavili postavitev cevovoda projekta, ki je napisan v programskem jeziku TypeScript in za gradnjo uporablja orodje pnpm. Projekt vsebuje interno razvito knjižnico za razvoj čelnih aplikacij z ogrodjem Angular. Obenem vsebuje tudi čelno aplikacijo, ki se uporablja za demonstracijske namene uporabe knjižnice, ki se povezuje na zaledni sistem, ki je del drugega projekta.

\subsection{Konfiguracija cevovoda}
Podobno kot pri prejšnem projektu, smo na konfiguracijski strežnik Consul dodali datoteko \texttt{config\_file} z naslednjo vsebino:
\begin{verbatim}
# Globalne spremenljivke
variables:
  KUBECONFIG_CONSUL_PATH: "default/.kube/config.v24"
...
Ponovno smo nastavili globalni spremenljivki \texttt{KUBECONFIG\_CONSUL\_PATH} in \texttt{EXPOSED\_SUFFIX} in dodali nastavitve servisa, ki predstavlja čelno aplikacijo. Servisu smo s ključno besedo \texttt{dir} spremnili privzeti imenik v repozitoriju v katerem se nahaja in dodali spremenljivko \texttt{coreBasePath}, ki jo potrebuje za namestitev. Nato smo pod ključno besedo \texttt{it} definirali še namestitvi, ki jih cevovod potrebuje za izvajanje integracijskih testov. Prva namestitev je čelna aplikacija, druga namestitev pa je zaledna aplikacija, s katero čelna aplikacija komunicira.

\subsection{Zagonske slike}
V imenik \texttt{runner-images} smo dodali datoteko Docker, ki definira zagonsko sliko imenovano \texttt{node-pnpm} za izvajanje nalog v cevovodu. Za osnovo, pod ključno besedo \texttt{FROM}, uporablja sliko \texttt{rocky-with-tools}. Nato sledijo ukazi za namestitev dodatnih orodji, ki jih potrebujemo za izvajanje nalog, med drugimi tudi Node in pnpm.

\subsection{Aplikacijska slika Docker}
Datoteka Docker, ki definira aplikacijsko sliko Docker za osnovo uporablja sliko \texttt{nginx}. Ostali ukazi, ki ji datoteka vsebuje, skopirajo vse potrebne datoteke za zagon aplikacije in nastavijo skript, ki se ob zagonu slike zažene.

\subsection{Konfiguracijske datoteke platforme Kubernetes}
Konfiguracijske datoteke za namestitev aplikacije na gručo platforme Kubernetes so zelo podobne tistim v prejšnjem projektu. Razlika je zgolj v tem, da se tu vse nahajajo v imenik \texttt{k8s}, saj vedno nameščamo aplikacijo na gručo iste verzije.
...
\end{verbatim}
V datoteki \texttt{.gitlab/.main.yml} pa uporabimo predlogo za cevovod projekta, ki uporablja okolje Node in orodje pnpm, nastavimo nekaj spremenljivk na vrednost \texttt{true}, kar v cevovod doda nekaj nalog:
\begin{itemize}
    \item \texttt{RELEASE\_PACKAGE} - objava paketa na centralni repozitorij Nexus.
    \item \texttt{RELEASE\_PACKAGE\_PUBLIC} - objava paketa na register npm.
...
\end{itemize}
in prilagodimo nalogo za gradnjo aplikacije tako, da v artefakte naloge zapakira dodatne datoteke. Celotna vsebina datoteke izgleda tako:
\begin{verbatim}
include:
  - project: 'medius/ci-tools/medius-cd'
    ref: v4.5.1
...
\subsection{Delovanje cevovoda}
Po zgrajenih zagonskih slikah s cevovodom za gradnjo zagonskih slik, lahko zaženemo cevovod aplikacije, ki ga lahko vidimo na sliki \ref{TODO} in vsebuje naslednje naloge:
\begin{itemize}
    \item priprava spremenljivk za integracijske teste
    \item priprava komponente Medius CD
    \item gradnja aplikacije
    \item statična analiza kode (ang. lint)
    \item testi enot
    \item gradnja slik Docker
    \item namestitev aplikacij v integracijsko okolje
    \item integracijskih testi
    \item preverjanje kakovosti kode
    \item izdaja slike Docker
    \item naloge za izdajo paketa na centralni repozitorij Nexus in register npm
    \item namestitev aplikacije na gručo Kubernetes
...
V tem poglavju bomo predstavili postavitev cevovoda projekta, ki vsebuje dve aplikaciji napisani v programskem jeziku Python, ki za gradnjo uporabljata orodje setuptools. Aplikaciji uporabljata knjižnico FastAPI, s katero izpostavljata končne točke za izvajanje poslovnih operacij.
...
Projekt vsebuje dva servisa, ki uporabljata različni sliki Docker. V konfiguracijski datoteki cevovoda \texttt{config\_file} smo spremenili privzeti poti do slik, saj se obe nahajata v korenskem imeniku projekta. Datoteka Docker, ki se uporablja za namestitev servisa \texttt{analyzer}, za osnovo uporablja sliko \texttt{python:3.9-slim} in je sestavljena iz več delov v katerih pripravi okolje, namesti odvisnosti ter zgradi aplikacijo in jo na koncu tudi zažene. Datoteka Docker, ki se uporablja za namestitev servisa \texttt{anonymizer}, deluje na enak način. Posebnost teh dveh datotek Docker je, da ne potrebujeta vnaprej zgrajene aplikacije, vendar se aplikacija zgradi v procesu gradnje slike.

\subsection{Konfiguracijske datoteke platforme Kubernetes}
Tudi tu so konfiguracijske datoteke za namestitev aplikacije podobne tistim v prejšnjima projektoma. Nahajajo se v imeniku \texttt{k8s} in se uporabljajo za oba servisa. Vse vrednosti, ki se morda razlikujejo med servisoma, so dodane kot spremenljivke, ki se pred namestitvijo na gručo Kubernetes nadomestijo s pravilnimi vrednostmi, kot smo opisali v poglavju \ref{subsec:namestitev-aplikacije}.

\subsection{Konfiguracijske datoteke cevovoda}
Ponovno smo dodali enake datoteke kot prej, vendar smo v datoteki \texttt{.gitlab/.build-runners.yml} uporabili predlogo za gradnjo zagonske slike \texttt{python}:
\begin{verbatim}
include:
  - project: 'medius/ci-tools/medius-cd'
    ref: v4.5.1
...
\subsection{Delovanje cevovoda}
Po zgrajenih zagonskih slikah s cevovodom za gradnjo zagonskih slik, lahko zaženemo cevovod aplikacije, ki ga lahko vidimo na sliki \ref{} in vsebuje naslednje naloge:
\begin{itemize}
    \item priprava komponente Medius CD
    \item gradnja slik Docker
    \item izdaja slik Docker
    \item namestitev aplikacij v različna okolja
    \item naloge za izdajo novih verzij
\end{itemize}
