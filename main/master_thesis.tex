...
\subsection{Git}
\label{subsec:git}
Git\footnote{\url{https://git-scm.com/}} je sistem za verzioniranje, ki se pogosto uporablja pri razvoju programske opreme. Omogoča, da več razvijalcev naenkrat dela na enaki izvorni kodi in razvijalcem pomaga slediti spremembam, ki so bile narejene \cite{loeliger2012version}.
...
\begin{itemize}
    ...
    \item Z nadzorom dostopa do artefaktov in dodatnimi pravili izboljša varnost procesa CI/CD.
    \item S hranjenjem različnih verzij artefaktov, ki so bili zgrajeni in nameščeni, izboljša sledljivost CI/CD procesa.
\end{itemize}

\subsection{Register slik Docker}
\label{subsec:register-slik-docker}
% TODO: Tole ni v redu
Register slik Docker je servis, ki omogoča hranjene in distribucijo slik Docker. V register lahko slike Docker naložimo ali pa jih iz registra prenesemo. Če je register javen, pomeni da dostop do njega ni omejen, če pa je privaten, pa je d.

Obstaja veliko javnih registrov, kot je na primer Docker Hub, ki je privzeti register in ga vzdržuje Docker, Quay.io, IBM Cloud container registry\footnote{\url{https://www.ibm.com/cloud/container-registry}} in drugi \cite{Anwar2018}.
...
Maven\footnote{\url{https://maven.apache.org/}} je orodje za avtomatizacijo gradnje projektov spisanih v jeziku Java. Razvijalcem pomaga avtomatizirati proces gradnje, testiranja in namestitve aplikacije. Uporablja deklarativen pristop, s katerim razvijalci naštejejo vse odvisnosti, vtičnike in ostale informacije, potrebne za gradnjo projekta, v eno datoteko v formatu xml. Ta datoteka se imenuje \texttt{pom.xml} \cite{Varanasi2019}.
...
Plug 'N' Play Package Manager\footnote{\url{https://pnpm.io/}} je prav tako orodje za upravljanje paketov, ki so napisani v programskem jeziku JavaScript in se uporabljajo v izvajalnem okolju Node.js. PNPM je alternativa orodju NPM, ki si prizadeva izboljšati efektivnost in učinkovitost namestitve in upravljanja paketov.

Za razliko od orodja NPM, ki uporablja centralizirani predpomnilnik paketov in namešča odvisnosti projekta v mapo poimenovano \texttt{node\_modules}, orodje PNPM uporablja virtualno shrambo. Namesto podvajanja odvisnosti za več projektov, jih orodje PNPM shrani na centralno mesto na datotečnem sistemu, kar omogoča, da si različni projekti odvisnosti delijo.
...
Setuptools\footnote{\url{https://pypi.org/project/setuptools/}} je knjižnica, ki poenostavi razvoj in distribucijo paketov v programskem jeziku Python. Vsebuje zbirko uporabnih funkcij in razširitev, ki nam pomagajo pri pakiranju, distribuciji in namestitvi paketov za programski jezik Python.

Glavne funkcionalnosti so:
\begin{itemize}
    \item Podpora za kreiranje paketov: knjižnica poenostavi proces kreiranje in gradnje paketov za programski jezik Python. Za definicijo meta podatkov o paketu, kot so ime, verzija, avtor, odvisnosti in vhodne točke uporablja datoteko \texttt{setup.py}. Na ta način razvijalcu omogoči, da definira strukturo projekta in določi vire in dodatne datoteke, ki so del projekta.
    \item Upravljanje odvisnosti: knjižnica s ključno besedo \texttt{requires} v skripti \texttt{setup.py} omogoča avtorjem paketa nastavitev odvisnosti, ki jih paket potrebuje. Knjižnica prav tako omogoča avtomatsko razrešitev in namestitev teh odvisnosti, kar poenostavi namestitev in uporabo paketa.
...
Glavne prednosti tehnologije Docker so:
\begin{itemize}
    \item Prenosljivost: vsebniki so neodvisni od operacijskega sistema, jih lahko zaženemo na kateremkoli sistemu, ki podpira Docker in zagotavljajo konsistentno obnašanje v različnih okoljih.
    \item Razširljivost: orodje Docker omogoča hitro razširljivost aplikacij s podvajanjem vsebnikov, ki izvajajo enako aplikacijo in porazdelitvijo obremenitve med njimi. 
    ...
    \item Upravljanje različic: orodje Docker z označevanjem vsebniških slik omogoča enostavno verzioniranje in vračanje na starejše verzije.
\end{itemize}

\subsection{Kaniko}
\label{subsec:kaniko}
% preveckrat beseda Docker
Kaniko\footnote{\url{https://github.com/GoogleContainerTools/kaniko}} je odprtokodno orodje za gradnjo vsebniških slik Docker iz datotek Docker znotraj vsebnika ali na gruči Kubernetes. Za gradnjo vsebniških slik ne potrebuje privilegiranega dostopa do prikritega procesa Docker (angl. \textit{Docker daemon}), kar predstavlja glavno prednost pred načinom gradnje vsebniških slik, ki ga uporablja platforma Docker.

Platforma Docker za gradnjo slik Docker uporablja prikriti proces Docker, ki potrebuje korenski dostop in visoke pravice na gostujočem sistemu. V nekaterih okoljih, kot je na primer Kubernetes ali sistem za neprekinjeno integracijo, zaradi varnostnih zahtev, uporabniki nimajo zahtevanih pravic za zagon prikritega procesa Docker.

Kaniko to težavo reši z uporabo samostojnega orodja, ki se zažene znotraj vsebnika in zgradi sliko Docker brez priviligiranega dostopa. Vse potrebne operacije, kot so: prenos osnovnih slik Docker, izvajanje ukazov za gradnjo slike in kreiranje končne vsebniške slike, izvede s pravicami, ki jih ima vsak navaden uporabnik gostiteljskega sistema. To nam omogoča, da lahko slike Docker brez privilegiranega dostopa zgradimo na gruči Kubernetes ali drugih platformah za orkestracijo.

\section{Gitlab}
\label{sec:gitlab}

Gitlab je spletna platforma za razvijalce in informacijske tehnologe (DevOps???), ki vsebuje velik nabor orodji za upravljanje in dostavo aplikacij. V osnovi se uporablja kot porazdeljen sistem za nadzor različic (angl. \textit{distributed version control system - DVCS}), v katerem lahko razvijalci gostijo svoje repozitorije izvorne kode, na podoben način kot v sistemu Git (\ref{subsec:git}). Glavne funkcionalnosti so:
\begin{itemize}
    \item Orodja za vodenje projekta: organiziranje projekta, sledenje nalogam, dodeljevanje nalog razvijalcem in določanje mejnikov.
...
Gitlab CI/CD je sistem za gradnjo in zagon cevovodov za neprekinjeno integracijo in dostavo aplikacij, ki je integriran v platformo Gitlab. Razvijalcem omogoča avtomatizacijo gradnje, testiranja in namestitve aplikacij in s tem olajša proces dostave programske opreme.

Temelji na konceptu cevovodov, ki jih predstavljajo zaporedja nalog. Vsak cevovod ima lahko več faz v kateri je lahko več nalog. Z razporejanjem nalog po fazah, zagotovimo pravilen vrstni red izvajanja nalog. Vsaka naloga pa ima lahko več akcij.

Cevovode definiramo v konfiguracijski datoteki \texttt{.gitlab-ci.yml}, v formatu YAML, ki se mora nahajati v korenskem direktoriju projekta. V tej datoteki specificiramo vse faze, naloge, akcije, ki jih naloge izvajajo, artefakte, ki jih naloge generirajo in potrebne odvisnosti ter okoljske spremenljivke.

Za izvajanje nalog skrbijo izvajalci nalog Gitlab. Izvajajo se lahko na lastni infrastrukturi ali na oblačnih virtualnih strojih, kar omogoča deljeno in paralelno izvajanje nalog in lahko znatno pohitri povprečen izvajalni čas cevovoda. Izvajalci nalog so različnih tipov glede na to, katerim projektom so na voljo:
\begin{itemize}
    \item deljeni, ki so na voljo vsem projektom, ki obstajajo na instanci platforme Gitlab
    \item skupinski, ki so na voljo samo projektom v specifični skupini
    \item projektni, ki so na voljo samo specifičnemu projektu
\end{itemize}
Izvajalce nalog lahko prilagodimo svojim potrebam z omejitvijo števila vzporedni nalog, ki jih lahko izvajajo, omejitvami virov, ki so jim na voljo, in okoljskimi spremenljivkami. Lahko jim dodelimo tudi oznake, s katerimi lahko na nalogo natančno določimo njihovo izvajanje.

Med posameznimi izvajalci nalog je zagotovljena izolacija, tako da se lahko naloge izvajajo neodvisno druga od druge. Hkrati je poskrbljeno za varnost z nadzorom dostopa in ustreznimi metodami verodostojnosti, ki zagotavljajo integriteto celotnega procesa.

\section{Kubernetes}
\label{sec:kubernetes}
Kubernetes je odprtokodna rešitev za orkestracijo vsebnikov, ki avtomatizira namestitev, povečanje instanc (scaling??? mogoče skaliranje) in upravljanje aplikacij, nameščenih v vsebnikih. Glavni gradniki platforme Kubernetes so:
\begin{itemize}
    ...
    \item Stroki (angl. pods) - vozlišča gostijo stroke (angl. \textit{pods}), kjer tečejo vsebniki, ki so komponente vsebniške aplikacije. Vsebniki, ki tečejo na istem stroku si delijo omrežje, imenski prostor in prostor za hrambo podatkov.
    \item Nabor podvajanj (angl. replica set) - njegovo naloga je ohraniti stabilen nabor kopiranih strokov, ki se lahko izvajajo kadarkoli. Uporablja se za zagotavljanje določenega števila enakih strokov.
    \item Storitve (angl. services) - abstrakcija, ki predstavlja nabor strokov in definira način dostopa do njih. Omogočajo mehanizme za upravljanje z obremenitvijo in odkrivanje storitev.
    \item Ingress - objekt API, ki upravlja zunanji dostop do storitev v gruči. Omogoča združevanje pravil usmerjanja v en sam vir in lahko izpostavi več storitev pod istim naslovom IP.
    \item Volumni (angl. volumes) - shramba podatkov, ki je neodvisna od življenjskega cikla vsebnika in na ta način omogoča dolgotrajno shrambo podatkov, tudi ob zaustavitvi ali ponovnem zagonu vsebnika.
    ...
    \item Namestitive (angl. deployments) - omogočajo upravljanje namestitve in posodobitev vmesnikov. Definiramo jih z deklarativno konfiguracijsko datoteko in skrbijo za ustrezno skaliranje, posodobitve brez zaustavitve (angl. rolling updates) in vračanje na starejše verzije (angl. rollback).
\end{itemize}
...
Consul je platforma za upravljanje s porazdeljenimi storitvami, ki omogoča odkrivanje storitev, upravljanje konfiguracije in preverjanje zdravja storitev. Ena izmed glavnih komponent platforme Consul je konfiguracijski strežnik Consul, ki omogoča hranjenje in upravljanje aplikacijske konfiguracije na centralen in lahko dostopen način.

Konfiguracijski strežnik Consul uporablja hierarhično podatkovno shrambo, v kateri so podatki shranjeni v parih ključ-vrednost. To pomeni, da vsakemu ključu pripada vrednost, ki je lahko v poljubnem formatu. Dostop do teh vrednosti, pa je omogočen preko programskega vmesnika ali orodja za ukazno vrstico.

\subsection{Jsonnet}
\label{subsec:jsonnet}

Jsonnet\footnote{\url{https://jsonnet.org/}} je funkcijski jezik za kreiranje in upravljanje kompleksnih podatkovnih struktur v formatu JSON. Omogoča bolj natančen in fleksibilen način definicije in organizacije struktur JSON v primerjavi z navadni formatom JSON. Ponavadi se uporablja za definicijo dinamičnih ali parametriziranih struktur JSON, saj za kreiranje datotek v formatu JSON omogoča uporabo spremenljivk, funkcij, pogojnih stavkov, aritmetike in zank.

Ena glavnih prednosti uporabe jezika Jsonnet je tudi njegova modularnost in možnost ponovne uporabe z definicijo komponent, ki jih lahko večkrat uporabimo v različnih objektih JSON. Na ta način lahko znatno povečamo doslednost konfiguracije skozi projekt.

Prav tako omogoča tudi uvažanje definicij JSON iz drugih datotek, kar omogoča sestavljanje kompleksnih objektov JSON in komentarje, ki olajšajo dokumentiranje konfiguracije.
...
IntelliJ IDEA je integrirano razvojno okolje za razvoj programske opreme, ki vsebuje veliko dodatnih orodji in funkcionalnosti, ki povečajo produktivnost in poenostavijo proces razvoja programske opreme. Podpira veliko število različnih programskih jezikov in nudi napredno pomoč pri pisanju kode, analizo kode ter pomoč pri refaktoriranju kode, ki razvijalcem pomaga pri pisanju bolj berljive in učinkovite kode. Okolje vsebuje tudi vgrajeno podporo za sisteme za verzioniranje, upravljanje projekta in obsežno podporo za ogrodja za testiranje. Pomembna lastnost razvojnega okolja, ki je ključna za neprekinjeno integracijo, pa predstavlja tudi dobra integracija z orodji za gradnjo kot sta Maven in Gradle, ki olajša upravljanje z odvisnostmi in gradnjo projekta.
