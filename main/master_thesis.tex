...
% Fitz
% Lepp
Zaradi visoke konkurence na trgu programske opreme, organizacije namenjajo veliko pozornosti pospešitvi razvoja in dostave kvalitetne programske kode~\cite{Phillips2015}. Neprekinjena integracija (ang. Continuous Integration - CI) je v industriji razvoja programske opreme razširjena razvojna praksa, ki vzpodbuja redno dodajanje in združevanje sprememb programske kode, na primer večkrat na dan~\cite{Shahin2017}. CI omogoča podjetjem, ki razvijajo programsko kodo, da skrajšajo čas in povečajo pogostost razvojnega cikla, izboljšajo kvaliteto programske kode in povečajo produktivnost razvojne ekipe~\cite{Fitzgerald2017}. Glavna dela CI sta avtomatska gradnja in testiranje programske opreme~\cite{Leppanen2015}.
...
Izraz DevOps predstavlja zbirko praks razvoja programske opreme, ki združuje mentaliteto razvoja s preostalimi funkcijami v organizaciji. Glavni poudarek pristopa DevOps je delitev odgovornosti med vse ekipe celotnega življenjskega cikla razvoja programske opreme. V okviru tega se meje med različnimi vlogami postopno zabrisujejo, saj člani operativne ekipe prevzemajo naloge, ki so tradicionalno bolj usmerjene v razvijalce, in člani razvojne ekipe počnejo enako. Čeprav se izraz DevOps najpogosteje povezuje z razvojem (ang. Development - Dev) in operacijami (ang. Operations - Ops) informacijske tehnologije, pa lahko ta pristop razširimo tudi na druge skupine znotraj organizacije, kot so varnost, kontrola kakovosti (ang. Quality Analysis - QA), upravljanje podatkovnih baz ter omrežne storitve. Zato je poleg programske opreme, ki deluje po specifikaciji, ključno dostaviti stranki tudi operativne funkcije za delo s kodo, enostavno namestitev aplikacij v več različnih okolji, možnost prenosa kode k drugemu izvajalcu in podobno~\cite{smith_2020}.

\section{Cilji in doprinosi}
\label{sec:cilji-in-doprinosi}

Namen magistrskega dela je predstaviti izzive pri razvoju poslovno kritičnih aplikacij in dobre prakse razvoja programske opreme, ki nam pomagajo pri reševanju teh izzivov. V delu predstavimo strategije in tehnologije, ki nam omogočajo postavitev razvojnega okolja v katerem lahko prakse DevOps izvajamo. Bolj podrobno opišemo proces neprekinjene integracije in dostave v okolje, kjer nimamo dostopa do omrežja stranke in posledično nimamo dostopa do izvajalnega okolja ter možnosti nameščanje programske opreme v izvajalno okolje. Na koncu predstavimo tudi razviti vtičnik Gitlab Template Lint, ki v integrirano okolje IntelliJ doda funkcionalnost, ki smo jo med razvojem komponente za neprekinjeno integracijo in dostavo pogrešali.
...
\begin{enumerate}
    ...
    \item Razviti odprtokodni vtičnik za integrirano okolje IntelliJ, ki znotraj okolja preverja konfiguracijske datoteke GitLab CI/CD in prikazuje združeno vsebino vseh dodanih predlog.
\end{enumerate}
...
V raziskavi~\cite{Azad2023} so na podlagi obstoječih člankov poskušali določiti deset kritičnih dejavnikov za uspešno vzpostavitev praks DevOps, ki so jih razdelili v tri skupine:
\begin{itemize}
...
\begin{itemize}
    ...
    \item uporaben produkt - pogostejša izdaja novih verzij omogoča hitrejšo povratno informacijo. Na ta način razvojna ekipa hitreje izve katere funkcionalnosti aplikacije so uporabne in si jih uporabniki želijo in katere ne in lahko razvoj temu primerno prilagodijo.
    ...
    \item kvalitetnejši produkt - z avtomatskimi testi je večina napak odkritih pred objavo kar poveča kvaliteto produkta.
    \item zadovoljstvo strank - zaradi zanesljivejše objave in kvalitetnejšega produkta so stranke bolj zadovoljne.
\end{itemize}

V~\cite{Pooya2023} predstavijo orodja za vzpostavitev CI/CD, ki se največ uporabljajo, glavne razloge za uporabo praks CI/CD, katere aktivnosti so največkrat avtomatizirane z uporabo orodji neprekinjene integracije in kaj so prednosti in slabosti posameznih orodji. Med najbolj uporabljenimi orodji za neprekinjeno integracijo navedejo GitHub Actions, Jenkins, GitLab CI/CD, CircleCI in Azure DevOps. Kot glavne razloge za vpeljavo orodji naštejejo večjo zanesljivost, boljšo kvaliteto kode, večjo produktivnost, hitrejšo dostavo produktov, zmanjšanje finančnih stroškov in dela, hitrejšo povratno informacijo ter večjo transparentnost procesa gradnje projekta. Aktivnosti, ki jih orodja največkrat avtomatizirajo so: gradnja projekta, zagon testov enot, zagon integracijskih testov, zagon testov od konca do konca (ang. end-to-end), analiza pokritosti kode s testi, analiza kvalitete kode, analiza varnosti, pakiranje in namestitev ter upravljanje z odvisnostmi. Kot najbolj pomembne funkcionalnosti orodji navedejo dobro integracijo s platformo, ki gostuje kodo, enostavnost uporabe, podporo za specifične operacijske sisteme in možnost namestitve na lastni infrastrukturi. Kot najbolj popularno orodje se izkaže GitHub Actions, predvsem zaradi dobre integracije s platformo GitHub, ki je zelo razširjena in priljubljena med razvijalci. Glavna prednost orodja GitLab CI/CD, ki ga tudi sami uporabimo in ga razlikuje od orodja GitHub Actions, pa je možnost namestitve na lastni infrastrukturi. Poleg tega nudi tudi dobro integracijo s platformo GitLab, ki jo uporabljamo, enostavno uporabo privatnih računalniških virov za zagon procesov in dobro podporo za vsebnike.

V~\cite{Donca2022} so vzporedno z nami razvili generator cevovoda neprekinjene integracije in dostave za gibčne (ang. agile) projekte programske opreme, ki na podoben način rešuje problematiko kreiranja cevovodov. V članku predlagajo rešitev v obliki generatorja, ki v repozitoriju Git avtomatsko prepozna datoteke potrebne za gradnjo in namestitev projekta in avtomatsko kreira cevovod, ki to tudi naredi. Generator je napisan v programskem jeziku Bash, kar omogoča lažjo integracijo v različne platforme, in pripravi konfiguracijsko datoteko na podlagi katere se kreira cevovod na platformi GitLab, ki jo uporabimo tudi sami. Tako kot mi, za gradnjo vsebniških slik uporabijo orodje Kaniko in aplikacije namestijo na gručo platforme Kubernetes. Kot glavne prednosti navedejo zmanjšanje potrebnega časa za pripravo cevovodov, izboljšana struktura cevovodov, zmanjšanje podvojenosti konfiguracije cevovodov, zmanjšanje finančnih stroškov, ker se razvijalcem ni potrebno ukvarjati s kreiranjem, upravljanjem, popravljanjem in nadzorom cevovodov ter zmanjšanje dvoumnosti glede principov CI/CD, kar poveča standardizacijo in kvaliteto procesov v podjetju. Bistvena razlika med našim pristopom in njihovim pa je, da oni za kreiranje cevovoda razvijejo generator medtem, ko mi cevovode sestavimo iz vnaprej pripravljenih predlog.
...
% Z uvedbo naštetih praks lahko v podjetju dosežemo hitrejše razvojne cikle, hitrejšo objavo aplikacij v produkcijsko okolje, boljšo kvaliteto programske opreme in usklajenost implementacije s poslovnimi zahtevami. Obenem razvojnim ekipam omogočimo hitrejšo odzivnost na povratno informacijo strank in na zahteve trga~\cite{ebert2016devops}.
...
\begin{figure}
    \begin{center}
        \includegraphics[width=11cm]{figures/monorepo.png}
    \end{center}
...
\subsection{Register slik vsebnikov}
\label{subsec:register-slik-vsebnikov}
Register slik vsebnikov je sistem za centralizirano hranjene in distribucijo slik vsebnikov, ki so podrobneje predstavljene v poglavju \ref{subsec:docker}. Služi kot sistem za upravljanje različic slik, v katerega lahko slike shranjujemo, jih delimo z drugimi in jih prenašamo~\cite{Anwar2018}.
...
Platforma je sestavljena iz orodji in programskih vtičnikov, ki jih lahko uporabimo za zagon analize kvalitete kode lokalno ali v cevovodu CI, ter spletnega vmesnika, ki prikazuje podrobna poročila in metrike kakovosti kode, kar razvojnim ekipam olajša spremljanje napredka in dodeljevanje nalog za izboljšanje kakovosti.
...
\begin{itemize}
    \item Vse spremembe v repozitoriju od zadnje izdane verzije stisnemo (ang. squash) v eno spremembo in jo dodamo na posebno vejo imenovano \texttt{release}, namenjeno verzioniranju izdaj.
    \item Vsem odgovornim v podjetju preko elektronske pošte avtomatsko sporočimo, da smo izdali novo verzijo.
    \item Naročnik zažene posebno nalogo, ki je del strankinega cevovoda in prenese spremembe v svoj repozitorij.
...
Med razvojem predlog konfiguracijskih datotek GitLab, ki so del komponente za neprekinjeno integracijo in dostavo, smo večkrat imeli težave s preverjanjem pravilnosti razvite predloge, saj integrirano razvojno okolje IntelliJ, v katerem smo komponento razvijali, ne vsebuje funkcionalnosti za preverjanje pravilnosti takšnih datotek. Predloge smo zato preverjali z orodjem platforme GitLab, ki je integrirano v spletno aplikacijo platforme, ali pa z odrivom spremembe na ločeno vejo v repozitoriju in preverjanjem statusa novo nastalega cevovoda. Oba načina sta precej nerodna, saj je med razvojem potrebno zapustiti integrirano okolje. Zato smo se odločili, da razvijemo vtičnik za integrirano okolje IntelliJ, s katerim dodamo funkcionalnost za preverjanje pravilnosti konfiguracijskih datotek GitLab.

Za začetek razvoja vtičnika smo uporabili predlogo za razvoj vtičnika platforme IntelliJ\footnote{\url{https://github.com/JetBrains/intellij-platform-plugin-template}}, ki je javno dostopna na platformi GitHub in uporablja programski jezik Kotlin. Predloga vsebuje vnaprej pripravljeno strukturo projekta, dodane odvisnosti, ki jih za razvoj vtičnika potrebujemo, ter konfiguracijsko datoteko za cevovod CI na platformi GitHub, ki vsebuje naloge za gradnjo vtičnika, izdajo nove verzije vtičnika, zagon testov in rotacijo vsebine v datoteki \texttt{CHANGELOG.md}, ki vsebuje opis vseh sprememb po različicah. Za gradnjo vtičnika je specifirano orodje Gradle.

\subsection{Ideja}
\label{sec:vtičnik-gitlab-template-lint-ideja}
Platforma GitLab za integracijo z drugimi aplikacijami izpostavlja programski vmesnik z arhitekturnim stilom REST. Ena izmed končnih točk, ki jih izpostavlja, je tudi končna točka za preverjanje konfiguracijskih datotek orodja GitLab CI/CD. Ideja vtičnika je bila zato preprosta: ko uporabnik v integriranem razvojnem okolju spremeni konfiguracijsko datoteko, pošlji zahtevek na končno točko za preverjanje konfiguracijskih datotek, iz odgovora programskega vmesnika preberi ali je konfiguracija pravilna in v primeru napake v integriranem okolju prikaži opozorilo.

\begin{figure}
    \begin{center}
        \includegraphics[width=8cm]{figures/gitlab-template-lint-ideja.drawio (2).png}
    \end{center}
\caption{Vizualizacija ideje vtičnika.}
\label{fig:monorepo}
\end{figure}

\subsection{Avtentikacija z vmesnikom}
Programski vmesnik platforme GitLab za avtentikacijo zahtevkov omogoča več načinov. V našem primeru se je za najbolj preprost in praktičen način izkazala avtentikacija z žetonom, ki ga uporabniki vtičnika lahko pridobijo na platformi GitLab in ga morajo vnesti v nastavitve vtičnika.

\subsection{Nastavitve vtičnika}
\label{sec:vtičnik-gitlab-template-lint-nastavitve}
Integrirano orodje IntelliJ za razvoj uporabniških vmesnikov vtičnikov nudi domensko specifičen jezik (ang. Domain-Specific Language - DSL) imenovan Kotlin UI, ki omogoča razvoj uporabniških vmesnikov v kodi napisani v programskem jeziku Kotlin. Slednje smo uporabili tudi sami, da smo oblikovali uporabniški vmesnik za nastavitve našega vtičnika, ki ga lahko vidimo na sliki \ref{fig:gitlab-template-lint-nastavitve}. Glavni nastavitvi vtičnika sta:
\begin{itemize}
    \item pogostost preverjanja (ang. lint frequency) - kako pogosto se pošlje zahtevek na programski vmesnik platforme GitLab. Možnosti so: ob vsaki spremembi v datoteki, ko se datoteka shrani in ko uporabnik ročno zažene akcijo za preverjanje.
    \item žeton za avtentikacijo s platformo GitLab (ang. Gitlab API token) - urejanje žetona za posamezen spletni naslov platforme GitLab je omogočeno preko tabele, v katero lahko uporabniki vnesejo več različnih žetonov za različne instance platforme GitLab.
\end{itemize}
...
\chapter{Rezultati}
\label{ch:rezultati}
V poglavju bomo najprej opisali, kako smo izboljšali pripravo izvajalnih slik. Nato bomo predstavili še eno prednost uporabe konfiguracijske datoteke \texttt{config\_file} in kako lahko z uporabo razvite komponente enostavno postavimo cevovod projekta za razvoj aplikacije ter projekta za razvoj programske knjižnice. Na koncu bomo pokazali uporabo razvite komponente na treh resničnih projektih, ki se še razvijajo ali pa so že v fazi produkcije in vsebujejo programje v različnih programskih jezikih ter imajo različne zahteve.
...
V poglavju \ref{subsec:datoteka-config-file} smo predstavili datoteko \texttt{config\_file}, ki nam omogoča konfiguracijo cevovoda z datoteko v formatu YAML. Poleg olajšanja vzdrževanja vseh spremenljivk, ki so potrebne za prilagoditev cevovoda projektnim specifikam, nam ta datoteka olajša tudi prilagajanje cevovoda stranki. Stranka največkrat uporablja svoj centralni repozitorij, konfiguracijski strežnik, register vsebniških slik in drugo. Hkrati se namestitvena okolja v podjetju in pri stranki razlikujejo. Konfiguracijska datoteka omogoča, da lahko stranka sama enostavno spremeni nastavitve svojega cevovoda. To znatno olajša razvoj aplikacije, saj konfiguracija cevovoda ni dodana v sistem za nadzor različic in za spremembo obnašanja cevovoda ni potrebno izdajati nove različice aplikacije.
...
\begin{itemize}
    ...
    \item Preverjanje kakovosti - naloga, ki z orodjem Maven zažene vtičnik platforme SonarQube za preverjanje kakovosti.
...
\begin{figure}
    \begin{center}
        \includegraphics[width=10cm]{figures/java-projekt-cevovod-izvajalne-slike.png}
    \end{center}
...
\subsection{Delovanje cevovoda}
Potem, ko smo zgradili izvajalne slike s cevovodom za gradnjo izvajalnih slik, lahko zaženemo cevovod aplikacije, ki vsebuje naslednje naloge:
\begin{itemize}
...
\begin{figure}
    ...
    \end{center}
\caption{podrejen cevovod za namestitev aplikacij v okolje za integracijsko testiranje.}
...
\subsection{Delovanje cevovoda}
Po zgrajenih izvajalnih vsebniških slikah lahko zaženemo cevovod aplikacije, ki vsebuje naslednje naloge:
\begin{itemize}
    \item Priprava komponente Medius CD
    \item Gradnja vsebniški slik - naloga zažene podrejen cevovod, ki ga lahko vidimo na sliki \ref{fig:python-projekt-build-docker-cevovod} in vsebuje nalogi za gradnjo obeh aplikacijskih slik.
    \item Izdaja vsebniških slik
    \item Namestitev aplikacij - naloga zažene podrejen cevovod, ki ga lahko vidimo na sliki \ref{fig:python-projekt-namestitev-aplikacij} in vsebuje naloge za namestitev aplikacij v različna okolja.
    \item Naloge za izdajo novih verzij
\end{itemize}
...
\begin{figure}
    ...
    \end{center}
\caption{podrejen cevovod za gradnjo vsebniških slik.}
...
Razvili smo komponento za vzpostavitev neprekinjene integracije in dostave na platformi GitLab imenovano Medius CD. Cilj komponente je poenotenje cevovodov, zmanjšanje podvojene konfiguracije in kode ter olajšanje vzdrževanja. To smo dosegli z uporabo konfiguracijskih datotek GitLab, ki definirajo naloge v cevovodu in skript napisanih v jeziku Bash, ki so v pomoč izvedbi konfiguriranih nalog. Uvedli smo nov način gradnje izvajalnih slik, ki olajša vzdrževanje le teh, in konfiguracijsko datoteko \texttt{config\_file}, ki olajša vzdrževanje spremenljivk, s katerimi prilagajamo cevovod. Na ta način lahko v cevovodu enostavno zgradimo več različnih aplikacijskih vsebniških slik, zaženemo integracijske teste in namestimo aplikacijo v različna okolja. Za enostavno postavitev cevovoda smo pripravili več datotek s predlogami GitLab za različne programske jezike, ki jih lahko v projektih uporabimo.
