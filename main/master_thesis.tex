...
Kaniko to težavo reši z uporabo samostojnega orodja, ki se zažene znotraj vsebnika in zgradi sliko Docker brez priviligiranega dostopa. Vse potrebne operacije, kot so: prenos osnovnih slik Docker, izvajanje ukazov za gradnjo slike in kreiranje končne vsebniške slike, izvede s pravicami, ki jih ima vsak navaden uporabnik gostiteljskega sistema. To nam omogoča, da lahko slike Docker zgradimo na gruči Kubernetes ali drugih platformah za orkestracijo, brez privilegiranega dostopa.
...
Temelji na konceptu cevovodov, ki jih predstavljajo zaporedja nalog. Vsaka naloga ima določeno fazo v kateri se izvede in akcije, kot so na primer: gradnja, testiranje ali namestitev aplikacije. Vsak cevovod ima lahko več faz v kateri je lahko več nalog. S fazami tako zagotovimo pravilen vrstni red izvajanja nalog.
...
Za izvajanje nalog skrbijo izvajalci nalog Gitlab. To so agenti, ki skrbijo za izvajanje nalog, ki so definirane v konfiguracijski datoteki. Nameščeni so na lastni infrastrukturi ali v oblačnih virtualnih strojih, kar omogoča deljeno in paralelno izvajanje nalog. Slednje omogoča izvajanje več nalog hkrati, kar lahko znatno pohitri povprečen izvajalni čas cevovoda.
...
\subsection{Vtičnik Gitlab Template Lint}
